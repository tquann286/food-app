                            ...
                            public void onComplete(@NonNull Task<AuthResult> task) {
                                ...
                                if(task.isSuccessful()){
                                    ...
                                    if(Fauth.getCurrentUser().isEmailVerified()){
                                        mDialog.dismiss();
                                        Toast.makeText(DeliveryLogin.this, "Đăng nhập thành công", Toast.LENGTH_SHORT).show();
                                        Intent Z = new Intent(DeliveryLogin.this,DeliveryFoodPanel_BottomNavigation.class);
                                        startActivity(Z);
                                        finish();

                                    }else{
                                        ReusableCodeForAll.ShowAlert(DeliveryLogin.this,"Đăng nhập thất bại","Bạn chưa xác thực email.");
                                    }
                                }else{
                                    mDialog.dismiss();
                                    ReusableCodeForAll.ShowAlert(DeliveryLogin.this,"Lỗi",task.getException().getMessage());
                                }
                            }
                        });
                    }
